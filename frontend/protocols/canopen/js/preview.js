...
let canDefinitions = {}; // CANopen消息定义
...
async function loadCanDefinitions() {
    try {
        const response = await fetch('/config/canopen/definitions.json');
        if (response.ok) {
            canDefinitions = await response.json();
        } else {
            console.warn('无法加载CANopen定义文件');
            canDefinitions = {};
        }
    } catch (error) {
        console.warn('加载CANopen定义失败:', error);
        canDefinitions = {};
    }
}
...
async function loadNameDefinitions() {
    try {
        const response = await fetch('/config/canopen/name_definitions.json');
        if (response.ok) {
...
async function loadRowHighlightConfig() {
    try {
        const response = await fetch('/config/canopen/row_highlight.json');
        if (response.ok) {
...
async function loadFromToMapping() {
    try {
        const response = await fetch('/config/canopen/from_to_mapping.json');
        if (response.ok) {
...
async function loadDataParserConfig() {
    try {
        const response = await fetch('/config/canopen/data_parser.json');
        if (response.ok) {
...
async function loadTableStyleConfig() {
    try {
        const response = await fetch('/protocols/canopen/config/table_style_config.json');
        if (response.ok) {
...
function showPreview(data, filename, protocol = 'CAN') {
    ...
    const getDescriptionById = (id) => {
        ...
        // 首先检查nameDefinitions（用于RTB等非CANopen消息）
    ...
    const isIdInDefinitions = (id) => {
        ...
        // 向后兼容：如果配置文件为空，显示所有数据
        // 检查 canDefinitions 是否为空
        const canDefsEmpty = !canDefinitions ||
            (typeof canDefinitions === 'object' && Object.keys(canDefinitions).length === 0);

        // 检查 nameDefinitions 是否为空
        const nameDefsEmpty = !nameDefinitions ||
            !nameDefinitions.definitions ||
            (typeof nameDefinitions.definitions === 'object' &&
                Object.keys(nameDefinitions.definitions).length === 0);

        // 如果两个配置都为空，接受所有ID
        if (canDefsEmpty && nameDefsEmpty) {
            console.log('CANopen: 配置文件为空，显示所有数据');
            return true;
        }
        ...
        // 检查十六进制形式
        if (canDefinitions && canDefinitions.hasOwnProperty(idLower)) {
            return true;
        }
    ...
    for (let i = 0; i < data.rows.length; i++) {
        ...
        // 解析 Buffer: 
        // CAN: string=2cf:8:[10 40 ff 37 48 c1 0a 00]
        // CANOPEN: string=4:8257:0:2:[06 00 ]
        let parsedId = '';
        let parsedData = '';
        if (buffer) {
            // 尝试匹配 CANOPEN 格式 (Node:Index:Sub:Len:[Data])
            // 提取 Index 作为 ID (GROUP 1)
            let m = buffer.match(/^\s*string=\d+:(\d+):\d+:\d+:\[(.*?)\]\s*$/);

            if (m) {
                // CANOPEN Index 是十进制的，转换为十六进制字符串
                const decId = parseInt(m[1], 10);
                if (!isNaN(decId)) {
                    parsedId = decId.toString(16).toUpperCase();
                } else {
                    parsedId = m[1];
                }
            } else {
                // 尝试匹配 CAN 格式 (ID:Len:[Data])
                m = buffer.match(/^\s*string=([0-9a-fA-F]+):\d+:\[(.*?)\]\s*$/);
                if (m) {
                    parsedId = m[1];
                }
            }
            ...
            if (m) {
                // 规范化数据字节为大写两位分隔 (匹配最后一个group，对于CANOPEN是group 2，对于CAN是group 2)
                parsedData = m[2]
                    .trim()
                    .split(/\s+/)
                    .map(b => b.toUpperCase())
